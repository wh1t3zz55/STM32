...
u8 RTC_Init_LSE(void)
{
	...
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x6060) //从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
	{
		...
		//RTC_Set(2017,3,6,0,0,0); //设置时间
		get_time();
		RTC_Set(2019,5,30,18,07,30);  //设置时间
...
{		 
	...
	{							
		RTC_Get();//更新时间  
		printf("RTC Time:%d-%d-%d %d:%d:%d%*s",calendar.w_year,calendar.w_month,calendar.w_date,calendar.hour,calendar.min,calendar.sec,5,"");//输出闹铃时间
		...
		OLED_ShowStr(40,0,Y,2);//年
		
		OLED_ShowStr(93,0," ",2);//正确显示个位数
		OLED_ShowStr(85,0,O,2);//月
		
		OLED_ShowStr(118,0," ",2);//正确显示个位数
		OLED_ShowStr(110,0,R,2);//日
		
		OLED_ShowStr(48,2," ",2);//正确显示个位数
		OLED_ShowStr(40,2,H,2);//小时
			
		OLED_ShowStr(79,2," ",2);//正确显示个位数
		OLED_ShowStr(70,2,M,2);//分钟
		
		OLED_ShowStr(113,2," ",2);//正确显示个位数
		OLED_ShowStr(105,2,S,2);//秒
		
		//memset(S, 0, sizeof S);;//清空秒钟数组，以正确显示个位数
...
void weekset()
{
	switch(calendar.week)
			{
				case 0:
					OLED_ShowStr(88,4," ",2);//正确显示个位数
					OLED_ShowStr(96,4," ",2);//正确显示个位数
					OLED_ShowStr(104,4," ",2);//正确显示个位数
